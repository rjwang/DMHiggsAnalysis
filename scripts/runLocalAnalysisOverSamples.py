...
for proc in procList :
	...
	mytag = mytag.replace("+","")
		...
		if(split==1):
	            	cfgfile=outdir +'/'+ dtag + suffix + '.cfg'
		else:
                        cfgfile=outdir +'/'+ dtag + suffix + '_' + str(segment) + '.cfg'
			...
			print('\033[33m submit2batch.sh -q'+queue+' -G'+queuelog+'/'+dtag+str(segment)+'.log'+' -R"' + requirementtoBatch + '" -J' + dtag + str(segment) + ' `which wrapLocalAnalysisRun.sh` ' + theExecutable + ' ' + cfgfile + '\033[0m')
			...
			count = count + 1
			if count % 20 == 0: SCRIPT_L.writelines('sleep 120\n\n')
